...
import { MATCHES, TEAMS, FORMATIONS } from '../data/wcdata.js'
...
const EVENT_TYPE = { g: ['GOAL', '#00e0c6'], y: ['YELLOW', '#f5c451'], r: ['RED', '#e8475e'], s: ['SHOT', '#5d6e80'] }
        ...
        return (
          <div key={key} style={{ display: 'grid', gridTemplateColumns: '1fr 190px 1fr', alignItems: 'center', gap: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 12 }}>
              <span style={mono({ fontSize: 13, color: 'var(--text-body)', minWidth: 46, textAlign: 'right' })}>{hv}{unit}</span>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <span style={mono({ fontSize: 13, color: 'var(--text-body)', minWidth: 46 })}>{av}{unit}</span>
            </div>
          </div>
        )
      })}
...
function TimelineTab({ m }) {
  return (
    <div style={{ padding: '24px 28px', position: 'relative' }}>
      ...
      {m.timeline.map((e, i) => {
        const [typeName, markerColor] = EVENT_TYPE[e.type] || ['', '']
...
function ShotMapTab({ m, h, a, shotTeam, setShotTeam }) {
  ...
  const totalXg = list.reduce((q, s) => q + s.xg, 0).toFixed(1)
  return (
    <div style={{ padding: '22px 28px' }}>
      ...
      <svg viewBox="0 0 360 230" style={{ width: '100%', background: 'rgba(0,224,198,.015)', border: '1px solid rgba(255,255,255,.07)', borderRadius: 12 }}>
        ...
        <rect x="8" y="62" width="52" height="106" fill="none" stroke="rgba(255,255,255,.1)" strokeWidth="1" />
        <rect x="8" y="90" width="22" height="50" fill="none" stroke="rgba(255,255,255,.1)" strokeWidth="1" />
        ...
        {list.map((s, i) => {
          ...
          return (
            <circle
              key={i}
              cx={(180 + ((s.x - 50) / 50) * 166).toFixed(1)}
              cy={(14 + (s.y / 100) * 202).toFixed(1)}
              r={(4 + s.xg * 20).toFixed(1)}
              fill={fill}
              stroke={stroke}
              strokeWidth="1.5"
              opacity={op}
...
function buildLine(code, isHome) {
  const tm = TEAMS[code] || {}
  const tmpl = FORMATIONS[tm.formation] || []
  return {
    formation: tm.formation,
    teamName: tm.name,
    color: tm.color,
    subs: tm.subs || [],
    players: tmpl.map((pp, i) => ({
      x: (isHome ? 12 + pp.w * 276 : 12 + (1 - pp.w) * 276).toFixed(1),
      y: (isHome ? 408 - pp.d * 180 : 12 + pp.d * 180).toFixed(1),
      label: (tm.xi || [])[i] || '',
      color: tm.color,
    })),
  }
...
function LineupsTab({ m, h, a }) {
  const lineH = buildLine(m.home, true)
  const lineA = buildLine(m.away, false)
  const teamBlock = (line, color) => (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 10 }}>
        <span style={{ width: 9, height: 9, borderRadius: 2, background: color }} />
        <span style={{ fontSize: 14, fontWeight: 600 }}>{line.teamName}</span>
  ...
  return (
    <div style={{ padding: '22px 28px', display: 'grid', gridTemplateColumns: '300px 1fr', gap: 26, alignItems: 'start' }}>
      <svg viewBox="0 0 300 420" style={{ width: 300, background: 'rgba(0,224,198,.015)', border: '1px solid rgba(255,255,255,.07)', borderRadius: 12 }}>
        ...
        {[lineA, lineH].map((line) =>
          line.players.map((p, i) => (
            <g key={line.teamName + i}>
              <circle cx={p.x} cy={p.y} r="9" fill="#0a1018" stroke={p.color} strokeWidth="2" />
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
        {teamBlock(lineH, h.color)}
        {teamBlock(lineA, a.color)}
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ animation: 'vfade .4s ease both', marginTop: 32, paddingBottom: 150 }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 22 }}>
        ...
        <span style={mono({ fontSize: 10.5, letterSpacing: '.12em', color: 'var(--text-muted)' })}>GROUP STAGE · {MATCHES.length} MATCHES</span>
      </div>
              ...
              >
                ...
                <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', gap: 8, width: '100%' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
                    <span style={mono({ fontSize: 9, letterSpacing: '.1em', color: '#6f8093' })}>{g.group}</span>
                    <span style={mono({ fontSize: 9, color: '#56657a' })}>{g.date}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                      <span style={{ width: 7, height: 7, borderRadius: 2, background: (TEAMS[g.home] || {}).color }} />
                      <span style={mono({ fontSize: 13, color: 'var(--text-body)' })}>{g.home}</span>
                    </div>
                    <span style={mono({ fontSize: 13, fontWeight: 700, color: '#fff' })}>
                      {g.score[0]}–{g.score[1]}
                    </span>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                      <span style={mono({ fontSize: 13, color: 'var(--text-body)' })}>{g.away}</span>
                      <span style={{ width: 7, height: 7, borderRadius: 2, background: (TEAMS[g.away] || {}).color }} />
                    </div>
                  </div>
                </div>
              </button>
        ...
        <div style={{ background: 'linear-gradient(180deg,rgba(255,255,255,.04),rgba(255,255,255,.01))', border: '1px solid rgba(255,255,255,.09)', borderRadius: 18, overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center', gap: 20, padding: '24px 28px 18px' }}>
            ...
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={mono({ fontSize: 9.5, color: '#6f8093', letterSpacing: '.1em', marginTop: 7 })}>
                FT · HT {m.ht[0]}–{m.ht[1]}
              </div>
            </div>
